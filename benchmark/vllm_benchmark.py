...
def run_vllm_benchmark(model, input_len, output_len, batch_size,
    ...
    llm = LLM(model=model, enforce_eager=not enable_cuda_graph,
...
def _run_mirage_once(model, input_len, output_len, batch_size, prompt):
    ...
    env = os.environ.copy()
    # DTK (DCU) toolchain — set by sourcing the DTK env.sh (ROCM_PATH/HIPCC/PATH)
    env.setdefault("ROCM_PATH", os.environ.get("ROCM_PATH", "/home/dockershare/dtk-26.04.2-beta-bugfix-0729-ubuntu20.04"))
    dtk_bin = os.path.join(env["ROCM_PATH"], "bin")
    if dtk_bin not in env.get("PATH", ""):
        env["PATH"] = dtk_bin + ":" + env.get("PATH", "")
    env["CXX"] = os.path.join(dtk_bin, "hipcc")
    env["HIPCC"] = os.path.join(dtk_bin, "hipcc")
    if "MIRAGE_HOME" not in env:
        env["MIRAGE_HOME"] = MIRAGE_ROOT
    # Align with the DCU fleet demo environment (MMAC path, no gang/CK FMHA)
    env.setdefault("USE_GANG", "0")
    env.setdefault("USE_CK_FMHA", "0")
...
def main():
    ...
    parser.add_argument(
        "--enable-cuda-graph", action="store_true",
        help="Enable CUDA graph for vLLM (default: enforce_eager=True, no CUDA graph)"
    )
    parser.add_argument(
